...
<?php if (empty($mantto->id)  || $user->hasPermission(\Sismaed\Entity\Usuario::EDIT_USER_ACCESS)): ?>
...
<form action="/mantto/editari" method="post">
    ...
    <label ><h3 class="th4">Mantenimiento: <?=$mantto->nombre ?? ''?>
	 ...
	 <label>Nombre: </label>
	<input type="text" id="unidaddesc" name="mantto[nombre]" value="<?=$mantto->nombre ?? ''?>">
 	<br>
	 <label>Descripción: </label>
	<input type="text" id="unidaddesc" name="mantto[desc]" value="<?=$mantto->desc ?? ''?>">
 	<br>
	 <label>Fecha: </label>
	<input type= datetime-local id="datetime-local" name="mantto[fecha]" value="<?=date("Y-m-d\TH:i:s",strtotime($mantto->fecha?? ''))?>">
 	<br>
	<label>Tipo: </label>
...
<form action="" method="post">
	...
	<h3 class="th4">Documento asociado a la falla(s)
      ...
      <br>Descripción: <input type="text" id="unidaddesc">
